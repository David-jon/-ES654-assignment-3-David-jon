...
class LogisticRegression:
	...
	def fit_and_crossvalidate_autograd_L1(self,X, y, n_iter = 1000, lr = 0.001, K_Fold = 3,reg_const = 0.01):
		
		def COST(theta, X, y):
			var = np.dot(-y.T,np.log(1/(1+ np.exp(-1* (np.dot(X,theta)))))) - np.dot((1-y).T,np.log(1-1/(1+ np.exp(-1* (np.dot(X,theta))))))
			#print('val:',theta._value)
			return np.sum(var+ reg_const* theta)/y.size
		def SIGMOID(theta, X):
		    return 1/(1+ np.exp(-1* (X@theta)))
		...
		for j in range(k):
		    ...
		    #print(abs(self.theta))
		    temp = abs(self.theta)>=0.0075
		    #print(temp)
		    imp_cols = []
		    for i1,i2 in enumerate(temp):
		    	if i2 >0:
		    		imp_cols.append(i1)
		    print('Important columns for fold',j,'is',imp_cols)
	...
	def fit_and_crossvalidate_autograd_L2(self,X, y, n_iter = 1000, lr = 0.001, K_Fold = 3,reg_const = 0.01):
		
		def COST(theta, X, y):
			return np.sum(reg_const* np.dot(theta,theta) + np.dot(-y.T,np.log(1/(1+ np.exp(-1* (np.dot(X,theta)))))) - np.dot((1-y).T,np.log(1-1/(1+ np.exp(-1* (np.dot(X,theta))))))+ reg_const* theta)/y.size
		def SIGMOID(theta, X): 
		    return 1/(1+ np.exp(-1* (X@theta)))
		grad_cost = grad(COST,0)
